      ...
      <div class="py-2 my-2 bg-white rounded-lg min-h-screen">
        <div class="filter-container">
          <div class="flex -mx-3 mb-6 p-3 md:flex-row flex-col w-full">
            <div class="w-full md:w-1/5 px-2 mb-6 md:mb-0">
              ...
              <x-jet-input id="cpea_id" class="form-control block w-full filter-field" type="text" name="cpea_id"
                :value="app('request')->input('cpea_id')" autofocus autocomplete="id" />
            </div>
            <div class="w-full md:w-1/5 px-2 mb-6 md:mb-0">
              <label class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2" for="customer_name">
                {{ __('Cliente') }}
              </label>
              <x-jet-input id="customer_name" class="form-control block w-full filter-field" type="text"
                name="customer_name" :value="app('request')->input('customer_name')" autofocus autocomplete="id" />
            </div>
            <div class="w-full md:w-1/2 px-2 mb-6 md:mb-0">
              <label class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2" for="product_id">
                {{ __('Produto') }}
              </label>
              <x-custom-multi-select multiple :options="$products" name="product_id[]" id="product_id" :value="[]" select-class="form-input filter-field" class="" no-filter="no-filter"/>
            ...
            <div class="w-full md:w-1/4 px-2 mb-6 md:mb-0">
              ...
              <x-custom-select class="mt-1" :options="$employees" name="employee_id" id="employee_id"
                :value="app('request')->input('employee_id')" />
            </div>
            <div class="w-full md:w-1/4 px-2 mb-6 md:mb-0">
        ...
        <div class="flex mt-4">
          <table id="conversation_items_table" class="table table-responsive md:table w-full">
            @include('conversations.cpea-ids.filter-result', ['conversationItems' => $conversationItems, 'ascending' =>
            $ascending, 'orderBy' => $orderBy])
          </table>
        </div>
  ...
  <script>
    window.addEventListener("load", function() {
      var filterCallback = function (event) {
        ...
        var product_idE = document.querySelectorAll("#product_id option:checked");
        const product_id = Array.from(product_idE).map(el => el.value);
