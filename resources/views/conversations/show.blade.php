    ...
    <div class="md:max-w-6xl lg:max-w-full mx-auto px-4">
      <div class="flex md:flex-row flex-col">
        <div class="w-full flex items-center">
          <h1>{{ __('Empresa') }} - {{ $conversation->customer->name }} - {{ __("Conversa") }} ({{
            str_pad($conversation->id, 5, 0, STR_PAD_LEFT) }})</h1>
        </div>
        <div class="w-full flex justify-end">
          <div class="m-2 ">
            <a class="btn-outline-danger"
              href="{{ route('customers.show', ['customer' => $conversation->customer_id]) }}">{{ __('Voltar') }}</a>
          </div>
        </div>
      </div>
        ...
        <div class="flex py-2 mt-4">
          <table class="table-items table table-responsive md:table w-full">
            ...
            <tbody>
              ...
              <tr>
                <td>
                  <a class="text-green-600 underline font-bold" href="{{ route(" customers.conversations.item.show",
                    ["item"=> $item->id]) }}">
                    {{ $item->order }}
                  </a>
                </td>
                ...
                <td style="white-space: nowrap;">{{ $item->interaction_at->format('d/m/Y H:i') }}</td>
                <td>{{ $item->user->full_name }}</td>
                <td>{!! $item->item_details !!}</td>
                <td>{{ $item->detailedContact ? $item->detailedContact->contact : '-' }}</td>
                <td style="white-space: nowrap;">
                  {{ $item->conversationStatus ? $item->conversationStatus->name : "-" }}
                </td>
                <td style="white-space: nowrap;">
                  @if($item->item_type == "Proposta")
                ...
                <td>
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                    stroke="currentColor"
                    class="w-6 h-6 @if(count($item->attachments) > 0) text-gray-700 @else text-gray-300 @endif">
