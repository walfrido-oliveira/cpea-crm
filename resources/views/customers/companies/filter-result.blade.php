<thead>
    <tr class="thead-light">
        ...
        <x-table-sort-header :orderBy="$orderBy" :ascending="$ascending" columnName="id" columnText="{{ __('Cód. Empresas/Filiais') }}"/>
        <x-table-sort-header :orderBy="$orderBy" :ascending="$ascending" columnName="customer_id" columnText="{{ __('Cliente') }}"/>
        <x-table-sort-header :orderBy="$orderBy" :ascending="$ascending" columnName="name" columnText="{{ __('Empresas/Filiais') }}"/>
        <x-table-sort-header :orderBy="$orderBy" :ascending="$ascending" columnName="addresses.city" columnText="{{ __('Cidade') }}"/>
        <x-table-sort-header :orderBy="$orderBy" :ascending="$ascending" columnName="addresses.state" columnText="{{ __('UF') }}"/>
...
<tbody id="customers_table_content">
    @forelse ($customers as $key => $customer)
        <tr>
            ...
            <td>
                <a class="text-item-table" href="{{ route('customers.show', ['customer' => $customer->id]) }}">{{ count($customer->addresses) ? $customer->addresses[0]->city : '-' }}</a>
            </td>
            <td>
                <a class="text-item-table" href="{{ route('customers.show', ['customer' => $customer->id]) }}">{{ count($customer->addresses) ? $customer->addresses[0]->state : '-' }}</a>
            </td>
            <td>
