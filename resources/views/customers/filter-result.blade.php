<thead>
  <tr class="thead-light">
    ...
    <x-table-sort-header :orderBy="$orderBy" :ascending="$ascending" columnName="id"
      columnText="{{ __('Cód. Cliente') }}" />
    <x-table-sort-header :orderBy="$orderBy" :ascending="$ascending" columnName="name"
    ...
    <x-table-sort-header :orderBy="$orderBy" :ascending="$ascending" columnName="created_at"
      columnText="{{ __('Data de Cadastro') }}" />
...
<tbody id="customers_table_content">
  ...
  <tr>
    <td>
      <input class="form-checkbox customers-url" type="checkbox" name="customers[{{ $customer->id }}]"
    ...
    <td>
      <a class="text-item-table" href="{{ route('customers.show', ['customer' => $customer->id]) }}">{{
        $customer->segment ? $customer->segment->name : '-' }}</a>
    </td>
    <td>
      <span
        class="w-24 py-1 @if($customer->is_new_customer) badge-success @elseif(!$customer->is_new_customer) badge-danger @endif">
    ...
    <td>
      <span class="w-24 py-1 @if($customer->status == " active") badge-success @elseif($customer->status == 'inactive')
        badge-danger @endif" >
        {{ __($customer->status) }}
      </span>
    </td>
    <td>
      ...
      <button class="btn-transition-danger delete-customers"
        data-url="{!! route('customers.destroy', ['customer' => $customer->id]) !!}">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
