        ...
        <div class="md:max-w-6xl lg:max-w-full mx-auto px-4">
            ...
            <div class="flex md:flex-row flex-col">
                ...
                <div class="w-full flex justify-end">
                    <div class="m-2 ">
                        <a class="btn-outline-info" href="{{ route('etapas.create') }}" >{{ __('Cadastrar') }}</a>
                    </div>
                    <div class="m-2">
                        <button type="button" class="btn-outline-danger delete-etapas" data-type="multiple">{{ __('Apagar') }}</button>
                    </div>
                </div>
            </div>
            ...
            <div class="py-2 my-2 bg-white rounded-lg min-h-screen">
                ...
                <div class="flex mt-4">
                    <table id="etapas_table" class="table table-responsive md:table w-full">
                        @include('etapas.filter-result', ['etapas' => $etapas, 'ascending' => $ascending, 'orderBy' => $orderBy])
                    </table>
                </div>
                <div class="flex mt-4 p-2" id="pagination">
                        {{ $etapas->appends(request()->input())->links() }}
                </div>
            </div>
        </div>
    </div>

    <x-modal title="{{ __('Excluir Etapa') }}"
             msg="{{ __('Deseja realmente apagar esse Etapa?') }}"
    ...
    <script>
        window.addEventListener("load", function() {
            ...
            function eventsDeleteCallback() {
                document.querySelectorAll('.delete-etapas').forEach(item => {
                item.addEventListener("click", function() {
                    if(this.dataset.type != 'multiple') {
                        ...
                        var urls = '';
                        document.querySelectorAll('input:checked.etapas-url').forEach((item, index, arr) => {
                            urls += item.value ;
