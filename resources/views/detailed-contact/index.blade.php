<x-app-layout>
    <div class="py-6 index-detailed-contact">
        <div class="md:max-w-6xl lg:max-w-full mx-auto px-4">
            ...
            <div class="flex md:flex-row flex-col">
                ...
                <div class="w-full flex justify-end">
                    <div class="m-2 ">
                        <a class="btn-outline-info" href="{{ route('detailed-contacts.create') }}" >{{ __('Cadastrar') }}</a>
                    </div>
                    <div class="m-2">
                        <button type="button" class="btn-outline-danger delete-detailed-contacts" data-type="multiple">{{ __('Apagar') }}</button>
                    </div>
                </div>
            </div>
            ...
            <div class="py-2 my-2 bg-white rounded-lg min-h-screen">
                ...
                <div class="flex mt-4">
                    <table id="detailed_contact_table" class="table table-responsive md:table w-full">
                        @include('detailed-contacts.filter-result', ['detailedContacts' => $detailedContacts, 'ascending' => $ascending, 'orderBy' => $orderBy])
                    </table>
                </div>
                <div class="flex mt-4 p-2" id="pagination">
                        {{ $detailedContacts->appends(request()->input())->links() }}
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        window.addEventListener("load", function() {
            var filterCallback = function (event) {
                ...
                var url = "{!! route('detailed-contacts.filter') !!}";
                ...
                ajax.onreadystatechange = function() {
                    if (this.readyState == 4 && this.status == 200) {
                        ...
                        document.getElementById("detailed_contact_table").innerHTML = resp.filter_result;
            ...
            function eventsFilterCallback() {
                ...
                document.querySelectorAll("#detailed_contact_table thead [data-name]").forEach(item => {
                    item.addEventListener("click", orderByCallback, false);
                });
            }

            function eventsDeleteCallback() {
                document.querySelectorAll('.delete-detailed-contacts').forEach(item => {
                item.addEventListener("click", function() {
                    if(this.dataset.type != 'multiple') {
                        ...
                        var urls = '';
                        document.querySelectorAll('input:checked.detailed-contacts-url').forEach((item, index, arr) => {
                            urls += item.value ;
