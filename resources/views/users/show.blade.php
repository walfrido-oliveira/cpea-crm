<x-app-layout>
      ...
      <div class="flex md:flex-row flex-col">
        ...
        <div class="w-full flex justify-end">
          ...
          @if(auth()->user()->hasRole('admin'))
            ...
            <div class="m-2">
              <button type="button" class="btn-outline-danger delete-user" id="user_delete" data-toggle="modal"
                data-target="#delete_modal" data-id="{{ $user->id }}">{{ __('Apagar') }}</button>
            </div>
          @endif
        </div>
      </div>
          ...
          <div class="flex flex-wrap">
            ...
            <div class="w-full md:w-1/2">
              <p class="text-gray-500 font-bold">
                <span class="w-24 py-1 @if($user->status == " active") badge-success @elseif($user->status ==
  ...
  <x-modal title="{{ __('Excluir usuário') }}" msg="{{ __('Deseja realmente apagar esse usuário?') }}"
    ...
    url="{{ route('users.destroy', ['user' => $user->id]) }}" redirect-url="{{ route('users.index') }}" />
