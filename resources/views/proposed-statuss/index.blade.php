<x-app-layout>
    <div class="py-6 index-proposed-status">
        <div class="md:max-w-6xl lg:max-w-full mx-auto px-4">

            <div class="flex md:flex-row flex-col">
                <div class="w-full flex items-center">
                    <h1>{{ __('Status Proposta') }}</h1>
                </div>
                <div class="w-full flex justify-end">
                    <div class="m-2 ">
                        <a class="btn-outline-info" href="{{ route('proposed-statuss.create') }}" >{{ __('Cadastrar') }}</a>
                    </div>
                    <div class="m-2">
                        <button type="button" class="btn-outline-danger delete-proposed-statuss" data-type="multiple">{{ __('Apagar') }}</button>
                    </div>
                </div>
            </div>
            ...
            <div class="py-2 my-2 bg-white rounded-lg min-h-screen">
                ...
                <div class="flex mt-4">
                    <table id="proposed_status_table" class="table table-responsive md:table w-full">
                        @include('proposed-statuss.filter-result', ['proposedStatuss' => $proposedStatuss, 'ascending' => $ascending, 'orderBy' => $orderBy])
                    </table>
                </div>
                <div class="flex mt-4 p-2" id="pagination">
                        {{ $proposedStatuss->appends(request()->input())->links() }}
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        window.addEventListener("load", function() {
            var filterCallback = function (event) {
                ...
                var url = "{!! route('proposed-statuss.filter') !!}";
                ...
                ajax.onreadystatechange = function() {
                    if (this.readyState == 4 && this.status == 200) {
                        ...
                        document.getElementById("proposed_status_table").innerHTML = resp.filter_result;
            ...
            function eventsFilterCallback() {
                ...
                document.querySelectorAll("#proposed_status_table thead [data-name]").forEach(item => {
                    item.addEventListener("click", orderByCallback, false);
                });
            }

            function eventsDeleteCallback() {
                document.querySelectorAll('.delete-proposed-statuss').forEach(item => {
                item.addEventListener("click", function() {
                    if(this.dataset.type != 'multiple') {
                        ...
                        var urls = '';
                        document.querySelectorAll('input:checked.proposed-statuss-url').forEach((item, index, arr) => {
                            urls += item.value ;
