...
<tbody id="products_table_content">
    @forelse ($products as $key => $products)
        <tr>
            <td>
                <input class="form-checkbox products-url" type="checkbox" name="products[{{ $products->id }}]" value="{!! route('products.destroy', ['product' => $products->id]) !!}">
            </td>
            <td>
            ...
            <td>
                ...
                <button class="btn-transition-danger delete-products" data-url="{!! route('products.destroy', ['product' => $products->id]) !!}">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
